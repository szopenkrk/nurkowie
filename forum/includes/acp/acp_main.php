...
class acp_main
{
	...
	function main($id, $mode)
	{
		...
		if ($avatar_dir = @opendir('http://www.studioej.pl/testowo/forum2/forum/' . $config['avatar_path']))
		{
			while (($file = readdir($avatar_dir)) !== false)
			{
				if ($file[0] != '.' && $file != 'CVS' && strpos($file, 'index.') === false)
				{
					$avatar_dir_size += filesize('http://www.studioej.pl/testowo/forum2/forum/' . $config['avatar_path'] . '/' . $file);
				}
			}
			closedir($avatar_dir);
