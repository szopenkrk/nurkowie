...
{
	//global $db, $auth, $user, $config, $phpEx, $template, $phpbb_root_path; //LPADLO COMM
    global $db, $auth, $user, $config, $phpEx, $template, $phpbb_root_path, $forum_location , $remoteServerDir; //LPADLO ADD
	...
	if (!empty($data['attachment_data']) && $data['post_id'] && in_array($mode, array('post', 'reply', 'quote', 'edit')))
	{
		...
		foreach ($data['attachment_data'] as $pos => $attach_row)
		{
			...
			{
        //logString($remoteServerDir . $config['upload_path'] . '/' . utf8_basename($orphan_rows[$attach_row['attach_id']]['physical_filename']));
        //LPADLO ADD
        if ($forum_location == 'R') {
         	// insert attachment into db
  				if (!@file_exists($remoteServerDir . $config['upload_path'] . '/' . utf8_basename($orphan_rows[$attach_row['attach_id']]['physical_filename'])))
  				{
            //logString('no nie istenieje');
  					continue;
  				}
        }  else {
